...
async def referral_callback(
    ...
    await call.answer()
    await referral(call.message, session, edit=True)
...
def register(router: Router) -> None:
    ...
    router.message.register(referral, Command("utc"))
    router.message.register(
        referral,
        Text(["Рефералы", "UTC ссылки", "UTM ссылки"]),
    )
    router.callback_query.register(referral_callback, Text('admin:refs'))
